...
class Solution:
    def reconstructMatrix(self, upper: int, lower: int, colsum: List[int]) -> List[List[int]]:
        n = len(colsum)
        cnt2 = colsum.count(2)
        if upper + lower == sum(colsum) and upper >= cnt2 and lower >= cnt2:  # 有解
            upper_one = upper - cnt2  # 0行独1的个数
            res = [[], []]
            for s in colsum:
                if s == 2:
                    res[0].append(1)
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.reconstructMatrix(
        9, 2, [0, 1, 2, 0, 0, 0, 0, 0, 2, 1, 2, 1, 2]))  # []
    print(solution.reconstructMatrix(2, 1, [1, 1, 1]))  # [[1,1,0],[0,0,1]]
