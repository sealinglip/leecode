...
class Solution:
    def checkIfPrerequisite(self, numCourses: int, prerequisites: List[List[int]], queries: List[List[int]]) -> List[bool]:
        # 下面的方法，如果只考虑增加依赖会TLE；防止超时要加上确定不依赖的课程也要记录
        ...
        def check(u: int, v: int) -> bool:
            ...
            if v in dependencies:
                if u in dependencies[v]:
                    return True
                elif (-u-1) in dependencies[v]:
                    return False

                dep = False
                for d in dependencies[v]:
                    if check(u, d):
                        dep = True
                        break
