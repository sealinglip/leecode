...
class AuthenticationManager:
    ...
    def generate(self, tokenId: str, currentTime: int) -> None:
        self._tokenDict[tokenId] = currentTime + self._timeToLive

    def renew(self, tokenId: str, currentTime: int) -> None:
        if tokenId in self._tokenDict:
            if self._tokenDict[tokenId] > currentTime:
                self._tokenDict[tokenId] = currentTime + self._timeToLive
            else:
                del self._tokenDict[tokenId]

    def countUnexpiredTokens(self, currentTime: int) -> int:
        cnt = 0
        for v in self._tokenDict.values():
            if v > currentTime:
                cnt += 1
        return cnt
