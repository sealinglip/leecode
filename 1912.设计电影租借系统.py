...
class MovieRentingSystem:
    ...
    def rent(self, shop: int, movie: int) -> None:
        item = (self.shops[shop][movie], shop)
        # 从可租列表中删除
        self.movies[movie].discard(item)
        # 增加到已租列表
        self.rentedMovies.add((item[0], item[1], movie))
        

    def drop(self, shop: int, movie: int) -> None:
        item = (self.shops[shop][movie], shop)
        # 从已租列表中删除
        self.rentedMovies.discard((item[0], item[1], movie))
        # 增加到可租列表
        self.movies[movie].add(item)
        

    def report(self) -> List[List[int]]:
        res = []
        for i in range(min(len(self.rentedMovies), 5)):
            rentedMovie = self.rentedMovies[i]
            res.append([rentedMovie[1], rentedMovie[2]])
        return res
...
if __name__ == "__main__":
    ...
    # Testcase 2
    print("Testcase 2")
    movieRentingSystem = MovieRentingSystem(10,[[0,418,3],[9,5144,46],[2,8986,29],[6,1446,28],[3,8215,97],[7,9105,34],[6,9105,30],[5,1722,94],[9,528,40],[3,850,77],[3,7069,40],[8,1997,42],[0,8215,28],[7,4050,80],[4,7100,97],[4,9686,32],[4,2566,93],[2,8320,12],[2,5495,56]])
    print(movieRentingSystem.search(7837)) # []
    print(movieRentingSystem.search(5495)) # [2]
    movieRentingSystem.rent(4,7100)
    print(movieRentingSystem.search(9105)) # [6, 7]
    print(movieRentingSystem.search(1446)) # [6]
    print(movieRentingSystem.report()) # [[4,7100]]
    print(movieRentingSystem.search(9869)) # []
