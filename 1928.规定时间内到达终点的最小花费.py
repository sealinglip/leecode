...
class Solution:
    def minCost(self, maxTime: int, edges: List[List[int]], passingFees: List[int]) -> int:
        ...
        # for x, y, t in edges:
        #     if y not in net[x] or net[x][y] > t:
        #         net[x][y] = t
        #         net[y][x] = t

        # 定义dp函数，找状态转移方程
        # dp(i, t)为从0到i，耗时为t的最小花费
        # dp(i, t) = min([dp(j, t - net[i][j]) for j in net[i]]) + passingFees[i]
        # dp(0, t) = passingFees[0] if t == 0
        # dp(i, t) = ∞ if i != 0 and t == 0
        N = len(passingFees)
        ...
        for t in range(1, maxTime + 1):
            # for i in range(N):
            #     tmp = [dp[j][t - net[i][j]]
            #            for j in net[i] if net[i][j] <= t]
            #     if tmp:
            #         dp[i][t] = min(tmp) + passingFees[i]

            # 此法效率更高
            for i, j, cost in edges:
                if cost <= t:
                    dp[i][t] = min(dp[i][t], dp[j][t - cost] + passingFees[i])
                    dp[j][t] = min(dp[j][t], dp[i][t - cost] + passingFees[j])
