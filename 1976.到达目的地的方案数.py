...
# 给你一个整数 n 和二维整数数组 roads ，其中 roads[i] = [ui, vi, timei] 表示在路口 ui 和 vi 之间有一条需要花费 timei 时间才能通过的道路。你想知道花费 最少时间 
...
class Solution:
    def countPaths(self, n: int, roads: List[List[int]]) -> int:
        ...
        shortest = [inf] * n # 记录最短路径
        shortest[0] = 0
        ways = [0] * n # 记录最短路径时的路径数
        ways[0] = 1

        hq = [(0, 0)] # 记录最短路径和城市号的优先队列
        while hq:
            t1, u = heappop(hq)
            if t1 > shortest[u]:
                continue
