...
class LockingTree:
    ...
    def __init__(self, parent: List[int]):
        self.parent = parent
        # 上锁状态：
        self._lock = [0] * len(self.parent)
        # 记录子
        self.children = [[] for _ in range(len(self.parent))]
    ...
    def upgrade(self, num: int, user: int) -> bool:
        ...
        if self.checkAndUnlockDescendant(num):
            self._lock[num] = user
            return True
    ...
    def checkAndUnlockDescendant(self, num: int) -> bool:
        l = self._lock[num] != 0
        self._lock[num] = 0  # unlock
        for c in self.children[num]:
            l |= self.checkAndUnlockDescendant(c)
        return l
