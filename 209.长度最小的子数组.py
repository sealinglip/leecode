...
@Author: Thomas Young
@Date: 2020-06-28 08:10:27
@LastEditors: Thomas Young
@LastEditTime: 2020-06-28 13:41:23
'''
#
...
class Solution:
    def minSubArrayLen(self, s: int, nums: List[int]) -> int:
        if not nums:
            return 0
        l, r, nums_len = 0, 0, len(nums) #左边界，右边界，上限
        total, closest, closest_len = 0, 0, float('inf')
        while l <= r and r <= nums_len:
            if total < s:
                if r < nums_len:
                    total += nums[r]
                    r += 1
