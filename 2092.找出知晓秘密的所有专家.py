...
class Solution:
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        meetingGrpByTime = defaultdict(list)
        for x, y, t in meetings:
            meetingGrpByTime[t].append((x, y))
        
        # 并查集
        uf = UnionFind(n)
        uf.union(firstPerson, 0)
        for t in sorted(meetingGrpByTime.keys()):
            for x, y in meetingGrpByTime[t]:
                uf.union(x, y)
            # 对于那些还不知道秘密的人，断开连接，恢复原状
            for x, y in meetingGrpByTime[t]:
                if not uf.isConnected(x, 0):
                    uf.group[x] = x
                    uf.group[y] = y
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.findAllPeople(6, [[0,2,1],[1,3,1],[4,5,1]], 1)) # [0,1,2,3]
