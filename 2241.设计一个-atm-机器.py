...
class ATM:
    ...
    def _withdraw(self, amount: int, banknotesCount: List[int]) -> bool:
        for i in range(4, -1, -1):
            if amount >= self.banknotes[i] and self.count[i] > 0:
                c = min(self.count[i], amount // self.banknotes[i])
                amount -= c * self.banknotes[i]
                banknotesCount[i] += c
        return amount == 0
