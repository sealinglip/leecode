...
class Solution:
    def peopleAwareOfSecret(self, n: int, delay: int, forget: int) -> int:
        MOD = 10 ** 9 + 7
        # 记dp[i][j]为第i天刚好有多少人知道秘密(j+1)天了
        # j > 0时有dp[i][j] = dp[i-1][j-1]
        # dp[i][0] = sum(dp[i-1][j] for j in range(delay-1, forget-1))
        # j ∈ [0, forget-1)
        # 又因为dp[i][*]只依赖dp[i-1][*]，所以状态可以压缩
        dp = []
        dp.append(1)
        ...
        for i in range(n-1):
            if len(dp) >= forget:
                dp.pop()
            if len(dp) >= delay:
                dp.insert(0, sum(dp[delay-1:]) % MOD)
            else:
                dp.insert(0, 0)
