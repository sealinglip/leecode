...
class NumberContainers:
    ...
    def change(self, index: int, number: int) -> None:
        ...
        if number not in self.reverseMap:
            self.reverseMap[number] = []
        heapq.heappush(self.reverseMap[number], index)
    ...
    def find(self, number: int) -> int:
        if number not in self.reverseMap:
            return -1
        # 读时合并清理垃圾数据
        sc = self.reverseMap[number]
        while sc and self.map[sc[0]] != number:
            heapq.heappop(sc)
        return sc[0] if sc else -1
