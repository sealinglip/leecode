...
class Solution:
    def bestClosingTime(self, customers: str) -> int:
        n = len(customers)
        vacancyCnt = [0] * n # vacancyCnt[i]为前(i+1)小时有多少小时没人来
        occupancyCnt = [0] * n # occupancyCnt[i]为customers[i:]有多少顾客
        accum = 0
        for i in range(n):
            if customers[i] == 'N':
                accum += 1
        ...
        for i in range(n-1, -1, -1):
            if customers[i] == 'Y':
                accum += 1
            occupancyCnt[i] = accum

        minCost = n
        time = 0
        for i in range(n+1):
            # 计算在i小时关门的代价
            cost = (0 if i == 0 else vacancyCnt[i-1]) + (occupancyCnt[i] if i < n else 0)
            if minCost > cost:
                minCost = cost
                time = i
