...
class Solution:
    def findCrossingTime(self, n: int, k: int, time: List[List[int]]) -> int:
        time.sort(key=lambda x: x[0] + x[2])  # 时间越长、效率越低、排得越后
        cur = 0
        wait_in_left, wait_in_right = [], []
        work_in_left, work_in_right = [], []
        for i in range(k):
            heappush(wait_in_left, -i)  # 一开始大家都在左边等活
        ...
        while 1:
            ...
            left_to_go = n > 0 and wait_in_left  # 需要去右边
            right_to_go = bool(wait_in_right)  # 需要去左边
            if not left_to_go and not right_to_go:
                next = inf  # 下一个时刻
                if work_in_left:
                    next = min(next, work_in_left[0][0])
                if work_in_right:
                    next = min(next, work_in_right[0][0])
                cur = next
                continue
            if right_to_go:
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.findCrossingTime(
