...
class Solution:
    def minCost(self, basket1: List[int], basket2: List[int]) -> int:
        ...
        for x in basket2:
            cnt[x] -= 1
        mi = min(min(basket1), min(basket2)) # 最小值

        needSwap = []
        for k, c in cnt.items():
            if c & 1: # 数量不能是奇数
                return -1
            needSwap.extend([k] * (abs(c) >> 1))

        mi <<= 1
        if needSwap:
            needSwap.sort()
            return sum(min(x, mi) for x in needSwap[:len(needSwap) >> 1])
        return 0
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.minCost([4,2,2,2], [1,4,1,2])) # 1
    print(solution.minCost([2,3,4,1], [3,2,5,1])) # -1
