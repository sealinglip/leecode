...
class Solution:
    def sumDistance(self, nums: List[int], s: str, d: int) -> int:
        ...
        # 直接求出d秒后所有球的位置
        pos = sorted(num + (1 if i == 'R' else -1)
                     * d for num, i in zip(nums, s))
        res = 0
        n = len(nums)
        for i in range(n - 1):
            res = (res + (pos[i+1] - pos[i]) * (i + 1) * (n - i - 1)) % MOD
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.sumDistance([-2, 0, 2], "RLL", 3))  # 8
    print(solution.sumDistance([1, 0], "RL", 2))  # 5
