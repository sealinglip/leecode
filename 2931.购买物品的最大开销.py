...
class Solution:
    def maxSpending(self, values: List[List[int]]) -> int:
        ...
        qu = []
        hq = [(values[i][n-1], i, n-1) for i in range(m)] # 小顶堆实现优先队列
        heapq.heapify(hq)
        while hq:
            ...
            j -= 1
            if j >= 0:
                heapq.heappush(hq, (values[i][j], i, j))

        res = 0
        for i, v in enumerate(qu, start=1):
            res += i * v
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.maxSpending([[8,5,2],[6,4,1],[9,7,3]])) # 285
    print(solution.maxSpending([[10,8,6,4,2],[9,7,5,3,2]])) # 386
