...
# 输出：1
# 解释：
# 一开始 s 包含 2 个不同的字符，所以无论我们改变哪个， 它最多包含 3 个不同字符，因此最多包含 3 个不同字符的最长前缀始终是所有字符，因此答案是1。
# 
# 示例 3：
# 输入：s = "xxyz", k = 1
# 输出：4
...
class Solution:
    def maxPartitionsAfterOperations(self, s: str, k: int) -> int:
        ...
        num = mask = count = 0 # 从左至右的前缀分段数、当前段掩码、当前段不同字符数量
        for i in range(n-1):
            b = 1 << (ord(s[i]) - ord('a'))
            if not (mask & b):
                count += 1
        ...
        num = mask = count = 0 # 从右向左的前缀分段数、当前段掩码、当前段不同字符数量
        ...
        res = 0
        for i in range(n):
            seg = left[i][0] + right[i][0] + 2 # 默认按中间会有两个分段计数，再根据实际情况进行相应地增减
            mask = left[i][1] | right[i][1]
            count = mask.bit_count()
            if left[i][2] == k and right[i][2] == k and count < 26:
                # 可以把i位置的字符改成跟前后相邻段都不同的字符，让中间区域变成三段
                seg += 1
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.maxPartitionsAfterOperations("aabaab", 3)) # 1
    print(solution.maxPartitionsAfterOperations("xxyz", 1)) # 4
