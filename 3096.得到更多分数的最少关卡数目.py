...
class Solution:
    def minimumLevels(self, possible: List[int]) -> int:
        preSum = [0] + list(accumulate(possible))
        n = len(possible)
        for i in range(1, n):
            if (preSum[i] << 1) - i > ((preSum[-1] - preSum[i]) << 1) - n + i:
                return i
        return -1
# @lc code=end
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.minimumLevels([1,0,1,0])) # 1
    print(solution.minimumLevels([1,1,1,1,1])) # 3
    print(solution.minimumLevels([0,0])) # -1
