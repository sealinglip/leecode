...
class Solution:
    def hasIncreasingSubarrays(self, nums: List[int], k: int) -> bool:
        ...
        #         return True
        #     prev = x
        cnt = prevCnt = 0 # 连续递增和前一段连续递增的元素个数
        for x in nums:
            if x > prev:
                cnt += 1
            else:
                ...
                cnt = 1
            prev = x
            if min(prevCnt, cnt) >= k or cnt >= (k << 1):
                return True
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.hasIncreasingSubarrays([19,5], 1)) # True
    print(solution.hasIncreasingSubarrays([1,8,9,2,7,3,4,5], 3)) # False
    print(solution.hasIncreasingSubarrays([2,5,7,8,9,2,3,4,3,1], 3)) # True
