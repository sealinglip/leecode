...
class Solution:
    def maxIncreasingSubarrays(self, nums: List[int]) -> int:
        prev = -inf
        res = cnt = prevCnt = 0 # 连续递增和前一段连续递增的元素个数
        for x in nums:
            if x > prev:
                cnt += 1
            else:
                res = max(res, min(prevCnt, cnt), cnt >> 1)
                prevCnt = cnt
                cnt = 1
            prev = x
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.maxIncreasingSubarrays([19,5])) # 1
    print(solution.maxIncreasingSubarrays([1,8,9,2,7,3,4,5])) # 2
