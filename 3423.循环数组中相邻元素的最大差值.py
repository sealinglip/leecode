...
class Solution:
    def maxAdjacentDistance(self, nums: List[int]) -> int:
        return max(max(abs(nums[i+1] - nums[i]) for i in range(len(nums)-1)), abs(nums[0] - nums[-1]))
        
# @lc code=end

if __name__ == "__main__":
    solution = Solution()
    print(solution.maxAdjacentDistance([1,2,4])) # 3
    print(solution.maxAdjacentDistance([-5,-10,-5])) # 5
