...
class Spreadsheet:
    ...
    def __init__(self, rows: int):
        self.valueMap = {}
        self.formulaPattern = r'^=([A-Z]?\d+)\+([A-Z]?\d+)$'
        

    def setCell(self, cell: str, value: int) -> None:
        self.valueMap[cell] = value
    ...
    def getValue(self, formula: str) -> int:
        m = re.match(self.formulaPattern, formula)
        if m:
            x = m.group(1)
            y = m.group(2)
            return (int(x) if x.isdigit() else self.valueMap.get(x, 0)) + (int(y) if y.isdigit() else self.valueMap.get(y, 0))
        else:
            return 0
