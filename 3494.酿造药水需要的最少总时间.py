...
class Solution:
    def minTime(self, skill: List[int], mana: List[int]) -> int:
        ...
        for i in range(m):
            curTime = 0
            for j in range(n):
                curTime = max(curTime, times[j]) + skill[j] * mana[i]
            times[-1] = curTime
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.minTime([1,5,2,4], [5,1,4,2])) # 110
    print(solution.minTime([1,1,1], [1,1,1])) # 5
    print(solution.minTime([1,2,3,4], [1,2])) # 21
