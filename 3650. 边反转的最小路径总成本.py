...
class Solution:
    def minCost(self, n: int, edges: List[List[int]]) -> int:
        ...
        for x, y, w in edges:
            graph[x].append((y, w))
            graph[y].append((x, w << 1))

        visited = [False] * n
        dist = [inf] * n # dist[i] 记录节点0到i的最短距离
        dist[0] = 0
...
if __name__ == '__main__':
    solution = Solution()
    print(solution.minCost(4, [[0,1,3],[3,1,1],[2,3,4],[0,2,2]])) # 5
    print(solution.minCost(4, [[0,2,1],[2,1,1],[1,3,1],[2,3,3]])) # 3
