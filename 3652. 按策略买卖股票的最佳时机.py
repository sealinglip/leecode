...
class Solution:
    def maxProfit(self, prices: List[int], strategy: List[int], k: int) -> int:
        ...
        # 什么都不改的情况下，计算利润
        preProfitSum = [0] + list(accumulate(p * s for p, s in zip(prices, strategy)))
        res = baseProfit = preProfitSum[-1]

        n = len(prices)
        # 逐个尝试
        half = k >> 1
        for i in range(n-k+1):
            # 调整范围为[i, i+k), 计算调整之后的利润，
            p = baseProfit - (preProfitSum[i+k] - preProfitSum[i]) + (preSum[i+k] - preSum[i+k-half])
            res = max(res, p)
...
if __name__ == "__main__":
    solution = Solution()
    print(solution.maxProfit([4,2,8], [-1,0,1], 2)) # 10
    print(solution.maxProfit([5,4,3], [1,1,0], 2)) # 9
    print(solution.maxProfit([5,8], [-1,-1], 2)) # 8