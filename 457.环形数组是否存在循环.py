...
Author: Thomas Young
Date: 2021-08-07 18:10:24
LastEditors: Thomas Young
LastEditTime: 2021-08-07 22:05:31
'''
#
...
class Solution:
    def circularArrayLoop(self, nums: List[int]) -> bool:
        ...
        for i in range(N):
            if nums[i] == 0:
                continue
            slow, fast = i, next(i) # 快慢指针
            while nums[slow] * nums[fast] > 0 and nums[slow] * nums[next(fast)] > 0:
                if slow == fast:
                    if slow == next(slow): # 一步成环的情况
                        break
                    return True
                slow = next(slow)
                fast = next(next(fast))
            # 成环但不达标的情况，标记刚才访问的节点
            visit = i
            while nums[visit] * nums[next(visit)] > 0:
                tmp = visit
                visit = next(visit)
                nums[tmp] = 0
        return False
...
if __name__ == "__main__":
    ...
    print(solution.circularArrayLoop([1, 1, 2])) # True
