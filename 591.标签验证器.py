        ...
        while p < n:
            ...
            if stack:
                ...
                if code[tagPos] == '/':
                    tagPos += 1
                    closeTag = True
                elif code[tagPos] == '!':  # 判断是不是CDATA
                    tagPos += 1
                    if code[tagPos:tagPos+7] == '[CDATA[':
                        cdataEnd = code.find(']]>', tagPos+7)
                        if cdataEnd != -1:
                            p = cdataEnd + 3
                            continue
                    return False

                tagEnd = code.find('>', tagPos)
                if tagEnd == -1:
                    return False
                tagName = code[tagPos:tagEnd]
                if validTag(tagName):
                    if closeTag:  # 结束标签
...
if __name__ == "__main__":
    ...
    print(solution.isValid("<A></A>  <B>    </B>"))  # True
    ...
    print(solution.isValid(
        "<DIV> closed tags with tag name  <B>123</B> </DIV>"))  # True
    print(solution.isValid(
        "<DIV> closed tags with invalid tag name in cdata  <![CDATA[[</1234567890> and<b></b>]]>  </DIV>"))  # True
