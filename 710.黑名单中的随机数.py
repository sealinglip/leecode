...
class Solution:
    ...
    def __init__(self, n: int, blacklist: List[int]):
        # 这样解会内存耗尽，来个映射表好了
        # self.ca = list(range(n))
        # right = n - 1
        # for b in sorted(blacklist, reverse=True):
        #     if b != right:
        #         self.ca[b] = self.ca[right]
        #     right -= 1
        # self.right = right
        self.__mapping = dict()
        right = n - 1
        for b in sorted(blacklist, reverse=True):
            if b != right:
                self.__mapping[b] = self.__mapping.get(right, right)
            right -= 1
        self.right = right

    def pick(self) -> int:
        r = random.randint(0, self.right)
        return self.__mapping.get(r, r)
