...
class Solution:
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        def getSortKey(log: str) -> str:
            firstSpacePos = log.find(' ')
            if firstSpacePos == -1:
                return (' ', ' ')
            elif log[firstSpacePos+1].isdigit():
                return ('0', '0')
            else:
                return (log[firstSpacePos:], log[0:firstSpacePos])

        logs.sort(key=lambda l: getSortKey(l))
        return logs
